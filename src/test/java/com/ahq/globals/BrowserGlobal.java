...
//    }
    ...
    @QAFTestStep(description = "I mouseover on {locator} and right click")
    public static void iMouseoverOnAndRightClick(String locator) {
        WebElement element = new WebDriverTestBase().getDriver().findElement(locator);
        Actions builder = new Actions(new WebDriverTestBase().getDriver());
        // Move to the bottom third of the element
        int yOffset = (element.getSize().getHeight() * 2) / 3;

        Action mouseOverHome = builder.moveToElement(element, element.getSize().getWidth() / 2, yOffset)
                .contextClick().build();
        mouseOverHome.perform();
    }
...
//    }
    ...
    @QAFTestStep(description = "I verify text not present in page {text}")
    public static boolean iVerifyTextNotPresentInPage(String text) throws Exception {
        List<WebElement> list = new WebDriverTestBase().getDriver().findElements(By.xpath("//*[contains(text(),'" + text + "')]"));

        boolean status = list.isEmpty();

        Validator.verifyTrue(status, "Text found! '" + text + "'", "Text not found! '" + text + "'");
        Reporter.logWithScreenShot("");
...
//
    ...
    @QAFTestStep(description = "I scroll to the top of the page")
    public static void iScrollToTheTopOfThePage() throws Exception {
        WebDriver driver = new WebDriverTestBase().getDriver();
        JavascriptExecutor js = (JavascriptExecutor) driver;
        try {
            long lastHeight = ((Number) js.executeScript("return window.pageYOffset")).longValue();
            while (true) {
                ((JavascriptExecutor) driver).executeScript("window.scrollTo(0, 0);");
                Thread.sleep(2000);

                long newHeight = ((Number) js.executeScript("return window.pageYOffset")).longValue();
                if (newHeight == lastHeight) {
                    break;
                }
                lastHeight = newHeight;
